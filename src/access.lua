...
function handle_callback( conf, callback_url )
    ...
    if code then
        ...
        if redirect_back then
            --Should always land here if no custom Loggedin page defined!
            ngx.log(ngx.ERR, "============test anchor point #4: " .. redirect_back)
            return ngx.redirect(redirect_back)
        else
            oidc_error = {status = ngx.HTTP_BAD_REQUEST, message = "failed to redirect"}
            return kong.response.exit(oidc_error.status, { message = oidc_error.message })
        end
    else
    ...
    ngx.log(ngx.ERR, "============test anchor point #5")
end
...
function _M.run(conf)
    ...
    if ngx.var.server_port == 80 or ngx.var.server_port == 443 then
        ...
        elseif pl_stringx.endswith(path_prefix, "/oauth2/callback") then
        ...
        if pl_stringx.endswith(path_prefix, "/") then
            path_prefix = path_prefix:sub(1, path_prefix:len() - 1)
            callback_url = ngx.var.scheme .. "://" .. ngx.var.host .. ":" .. ngx.var.server_port .. path_prefix .. "/oauth2/callback"
        elseif pl_stringx.endswith(path_prefix, "/oauth2/callback") then
            --We are in the callback of our proxy
    ...
    if encrypted_token then
        ...
        if not access_token then
            ngx.log(ngx.ERR, "============broken access token: ", access_token)
            -- broken access token
            return redirect_to_auth( conf, callback_url )
        end
        ...
        if conf.user_info_cache_enabled then
            ...
            if userInfo then
                ...
                ngx.req.set_header("Authentication", "Bearer " .. access_token)
                return
            end
        end
